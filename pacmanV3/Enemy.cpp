...
void Enemy::update(float & CurrentFrame, float time)
{
	...
	else  sprite.setTextureRect(sf::IntRect(0, 0, w, h));
	x += dx*time;
	y += dy*time;
...
void Enemy::animation(float &CurrentFrame, float time)
{
	CurrentFrame += 0.0005*time;
	if (CurrentFrame > 3) CurrentFrame -= 2;
	sprite.setTextureRect(sf::IntRect(16 * int(CurrentFrame), 0, 16, 16));
}
