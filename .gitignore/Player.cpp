...
Player::Player(String F, float X, float Y, float W, float H):Entity(F, X, Y,  W, H)
{
	state = stay;
	prevState = stay;
	CookieCount = 0;
...
void Player::update(float &CurrentFrame,float time)
{
	...
	if (!CollideTheWall()) {
		...
		std::cout << "cOOllsdllsdl" << std::endl;
...
void Player::animation(float &CurrentFrame, float time,stateE state)
{
	...
	case left:
		...
		sprite.setTextureRect(IntRect(16 * int(CurrentFrame), 16, 16, 16)); break;
	case right:
			CurrentFrame += 0.005*time;
		if (CurrentFrame > 3) CurrentFrame -= 3;
		sprite.setTextureRect(IntRect(16 * int(CurrentFrame), 32, 16, 16)); break;
	case up:
			CurrentFrame += 0.005*time;
		if (CurrentFrame > 3) CurrentFrame -= 3;
		sprite.setTextureRect(IntRect(16 * int(CurrentFrame), 48, 16, 16)); break;
	case down:
			CurrentFrame += 0.005*time;
		if (CurrentFrame > 3) CurrentFrame -= 3;
		sprite.setTextureRect(IntRect(16 * int(CurrentFrame), 0, 16, 16)); break;
	}
